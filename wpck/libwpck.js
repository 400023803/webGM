function packWPCK (structure) {
    ...
    let fileBufWriter = new FileBufWriter(new ArrayBuffer(fileSize))
        ...
        fileBufWriter.int(0x09, 0x01, {endian, size: IntSize.U8})
        fileBufWriter.int(0x0A, items.length, {endian, size: IntSize.U16})
        for (let i = 0x00; i < items.length; i++) {
            let offset = itemBlockOffsets[i]
                fileBufWriter.int(0x0C + (IntSize.U32 * i), offset, {endian, size: IntSize.U32})
            let dataOffset = itemDataOffsets[i]
            let itemBlock = itemBlocks[i]
                fileBufWriter.int(offset, itemBlock.blockSize, {endian, size: IntSize.U8})
                fileBufWriter.int(offset + 0x01, i, {endian, size: IntSize.U16})
                fileBufWriter.int(offset + 0x03, itemBlock.pathLength, {endian, size: IntSize.U16})
                for (let j = 0x00; j < itemBlock.pathLength; j++) fileBufWriter.int((offset + 0x05) + (j * IntSize.U16), itemBlock.path[j], {endian, size: IntSize.U16})
                    offset += itemBlock.path.length * IntSize.U16
                fileBufWriter.int(offset + 0x05, itemBlock.nameLength, {endian, size: IntSize.U8})
                fileBufWriter.str(offset + 0x06, itemBlock.name)
                    offset += itemBlock.nameLength
                fileBufWriter.int(offset + 0x06, itemBlock.typeId, {endian, size: IntSize.U8})
                fileBufWriter.int(offset + 0x07, itemBlock.dataLength, {endian, size: IntSize.U32})
                fileBufWriter.int(offset + 0x0B, itemBlock.typeId == 0x00 ? dataOffset : 0x00, {endian, size: IntSize.U32})
